...
--
-- COMPLETED 2026-08-23: query 1 returned zero rows on both staging and production.
-- No live leak; the 2.3E fix is preventive. Kept here for reference only.
--
-- Run via Railway (pin the environment explicitly; never print the connection string):
--   railway run -e staging    --service Postgres bash -c 'psql "$DATABASE_PUBLIC_URL" -f archive/phase-2.3e-leak-check.sql'
--   railway run -e production --service Postgres bash -c 'psql "$DATABASE_PUBLIC_URL" -f archive/phase-2.3e-leak-check.sql'
--
